...
simulate <- sapply(nList, function(n) {
    net <- paste0("TS_", n[1], "To", n[length(n)])
    net1 <- paste0("TS1_", n[1], "To", n[length(n)])
    net2 <- paste0("TS2_", n[1], "To", n[length(n)])
    dir.create(net, showWarnings = F)
    file.copy("TS.topo", paste0(net, "/", net, ".topo"))
    ...
    sapply(nets, function(Net) {
        ...
        pr <- paste0(Net, ".prs")
        # cm <- paste0("wsl python3 ../convert_racipe_parameters.py ", par, " ", pr)
        # system(cm)

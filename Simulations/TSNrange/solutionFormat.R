...
solutionFileProcess <- function(net, tol = 1e-3, paramIDs = T)
{
    ### Adding parameter indices ----
    Net <- net %>% str_remove("\\d") %>% str_remove("_.*")
    solnNames <- c("parIndex", "nStatesRAC", "basin", "A", "B")
    # solnFile <- paste0(net,"_solution.dat")
    solnFile <- read_delim(paste0(net, "_solution.dat"), 
                            delim = "\t",
                            col_names = solnNames, 
                           col_types = cols())
    ...
    parIds <- data.frame(parIndex = pars,
        ...
        mutate(DSGRNPhase = str_split(DSGRNPhase, "\t") %>%
                            lapply(sort) %>% sapply(paste0, collapse = "-"))

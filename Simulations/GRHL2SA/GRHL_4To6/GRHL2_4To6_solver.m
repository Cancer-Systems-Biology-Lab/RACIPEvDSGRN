...
prs_file = 'GRHL2_4To6.prs';
prs_new = 'GRHL2_4To6.dat';
...
par_file = 'GRHL2_4To6_parameters.dat';
parameters = table2array(readtable(par_file));
parameters =  parameters(:, 3:end);
initFile = 'GRHL2_4To6_parameters_eqval.dat';
fid = fopen(initFile);
initConds = strings;
tline = fgetl(fid);
while ischar(tline)
...
parfor i = 1:size(parameters,1)
    ...
    for j = 1:l
        ...
        [t y] = ode23s(@(t,y)GRHL2_4To6(t,y,p), tspan, y0);
        timeCount = 0;
...
writetable(ss_tot, 'GRHL2_4To6_ss.csv');

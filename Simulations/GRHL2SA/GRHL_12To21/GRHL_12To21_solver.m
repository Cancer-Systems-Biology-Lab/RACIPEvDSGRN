...
prs_file = 'GRHL_12To21.prs';
prs_new = 'GRHL_12To21.dat';
...
par_file = 'GRHL_12To21_parameters.dat';
parameters = table2array(readtable(par_file));
parameters =  parameters(:, 3:end);
initFile = 'GRHL_12To21_parameters_eqval.dat';
fid = fopen(initFile);
initConds = strings;
tline = fgetl(fid);
while ischar(tline)
...
parfor i = 1:size(parameters,1)
    ...
    for j = 1:l
        ...
        [t y] = ode23s(@(t,y)GRHL_12To21(t,y,p), tspan, y0);
        timeCount = 0;
...
writetable(ss_tot, 'GRHL_12To21_ss.csv');

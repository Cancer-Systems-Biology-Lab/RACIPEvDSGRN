...
prs_file = 'GRHL1_47To56.prs';
prs_new = 'GRHL1_47To56.dat';
...
par_file = 'GRHL1_47To56_parameters.dat';
parameters = table2array(readtable(par_file));
parameters =  parameters(:, 3:end);
initFile = 'GRHL1_47To56_parameters_eqval.dat';
fid = fopen(initFile);
initConds = strings;
tline = fgetl(fid);
while ischar(tline)
...
parfor i = 1:size(parameters,1)
    ...
    for j = 1:l
        ...
        [t y] = ode23s(@(t,y)GRHL1_47To56(t,y,p), tspan, y0);
        timeCount = 0;
        while sum(round((y(end, :) - y((end-1), :)), 3)) ~= 0 && timeCount < 100000
            y0 = y(end,:);
            [t y] = ode23s(@(t,y)GRHL1_47To56(t,y,p), tspan, y0);
...
writetable(ss_tot, 'GRHL1_47To56_ss.csv');

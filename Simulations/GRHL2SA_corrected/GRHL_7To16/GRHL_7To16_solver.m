...
prs_file = 'GRHL_7To16.prs';
prs_new = 'GRHL_7To16.dat';
...
par_file = 'GRHL_7To16_parameters.dat';
parameters = table2array(readtable(par_file));
parameters =  parameters(:, 3:end);
initFile = 'GRHL_7To16_parameters_eqval.dat';
fid = fopen(initFile);
initConds = strings;
tline = fgetl(fid);
while ischar(tline)
...
parfor i = 1:size(parameters,1)
    ...
    for j = 1:l
        ...
        [t y] = ode23s(@(t,y)GRHL_7To16(t,y,p), tspan, y0);
        timeCount = 0;
...
writetable(ss_tot, 'GRHL_7To16_ss.csv');

...
prs_file = 'GRHL1_10To12.prs';
prs_new = 'GRHL1_10To12.dat';
...
par_file = 'GRHL1_10To12_parameters.dat';
parameters = table2array(readtable(par_file));
parameters =  parameters(:, 3:end);
initFile = 'GRHL1_10To12_parameters_eqval.dat';
fid = fopen(initFile);
initConds = strings;
tline = fgetl(fid);
while ischar(tline)
...
parfor i = 1:size(parameters,1)
    ...
    for j = 1:l
        ...
        [t y] = ode23s(@(t,y)GRHL1_10To12(t,y,p), tspan, y0);
        timeCount = 0;
...
writetable(ss_tot, 'GRHL1_10To12_ss.csv');

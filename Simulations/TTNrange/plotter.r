...
netTest <- nList3[[1]]
netTest <- paste0("TT_", netTest[1], "To", netTest[length(netTest)])
setwd(netTest)
solutions <- list.files(".", "solutionFormatter") %>%
    lapply(read_csv) %>%
    reduce(rbind.data.frame) %>%
    mutate(nStatesDSGRN = str_count(DSGRNPhase, "-") + 1) %>%
    select(nStatesDSGRN) %>% unlist %>% table %>% data.frame %>%
    set_names(c("nState", "Frequency")) %>%
    mutate(Frequency = Frequency/sum(Frequency))
ggplot(solutions, aes(x = reorder(nState, -Frequency),y = Frequency)) + 
    geom_bar(stat = "identity") +
    theme_Publication() + labs(x = "# States")
ggsave("../DSGRNPhaseSizes.png", width = 5.5, height = 5)
     
nStates <- solutions %>%
    mutate(Frequency = Frequency %>% cumsum) %>%
    filter(Frequency < 1) %>%
    select(nState) %>% unlist
nStates <- 1:3
...
sapply(names(nLists), function(nListName) {
    ...
    df  <- lapply(1:length(nList), function(i) {
                ...
                xLab <- xLabs[[i]]
                setwd(net)
                key <- str_remove(net, "TT_")
                keys <- paste0(c("TT","TT1", "TT2"), "_",key)
    ...
    sapply(params, function(p) {
        ...
        dfFil <- df %>% filter(Parameter == p) %>% filter(nState %in% nStates)
        ggplot(dfFil, aes(x = HillRange, y = Avg, fill = nState)) +
    ...
    setwd("..")
    write_csv(df, paste0("TT_", nListName, "_allData.csv"))    
})

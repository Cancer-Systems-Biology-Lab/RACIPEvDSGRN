...
converter <- function(topo_file){#browser()
    ...
    geneQuotes <- paste0("'", genes, "'") %>% paste0(collapse = " ")
    
    Hilleqns <- character()
    Parameters <- character()
    
    dummy <- sapply(df %>% t %>% as.data.frame, function(x){#browser()
        p <- hill_shift(x)
        Hilleqns <<- c(Hilleqns, p[[1]])
        Parameters <<- c(Parameters, p[[2]])
        NULL
    })
    ...
    parameter_read <- c(paste0("par_file = '", str_remove(topo_file, ".topo"), "_parameters.dat';"),
                        ...
                        paste0("initFile = '", str_remove(topo_file, ".topo"), "_parameters_eqval.dat';"),
                        "fid = fopen(initFile);",
                        "initConds = strings;",
                        "tline = fgetl(fid);",
                        "while ischar(tline)",
                        ...
                        "        timeCount = 0;",
                        paste0("        while sum(round((y(end, :) - y((end-1), :)), 3)) ~= 0 && timeCount < 100000"),
                        "            y0 = y(end,:);",
                        ...
                        paste0("ss_tot.Properties.VariableNames = {'ParIndex' ", geneQuotes, " 'Count'};"),

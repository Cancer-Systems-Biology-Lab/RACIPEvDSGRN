...
cmd <- function(topoFile ,nL) {
    paste0("racipemt ", topoFile, " -threads 48 -num_paras 10000 -num_ode 1000 -minN ", nL[1], " -maxN ", nL[length(nL)])
}

simulate <- sapply(nList, function(n) {
    net <- paste0("TSSA_", n[1], "To", n[length(n)])
    net1 <- paste0("TSSA1_", n[1], "To", n[length(n)])
    net2 <- paste0("TSSA2_", n[1], "To", n[length(n)])
    dir.create(net, showWarnings = F)
    file.copy("TSSA.topo", paste0(net, "/", net, ".topo"))
